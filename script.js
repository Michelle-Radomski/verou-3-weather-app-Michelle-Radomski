...
const processData = (data, xHours, temperature) => {
    for (let i = 0; i < 5; i++) {
        //the length of daily is 5 days instead of 8
        addCard(data, i);
    }
...
const addCard = (data, i) => {
    ...
    forecastDate.innerHTML = getDate(dailyData); //gives us day of the week
    ...
    weatherIconURL.src =
        "https://openweathermap.org/img/wn/" + weatherIcon + "@2x.png"; //gets image of icon
...
const getDayName = (dailyData) => {
    ...
    const day = new Date(unixTime * 1000).toLocaleString("en-US", {
        weekday: "long",
    });
    return day;
};

const getDate = (dailyData) => {
    const unixTime = dailyData.dt;
    const date = new Date(unixTime * 1000).toLocaleString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
    });
        ...
        datasets: [
            {
                ...
                data: "",
                backgroundColor: ["rgba(247, 136, 18, 1)"],
                borderColor: ["rgb(247, 136, 18, 1)"],
                borderWidth: 3,
            },
        ],
    },
